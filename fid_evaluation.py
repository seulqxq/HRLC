...
def output_real_images(dataloader, num_imgs, real_dir):
    ...
    for i in range(num_imgs//batch_size):
        ...
        for img in real_imgs:
            save_image(img, os.path.join(real_dir, f'{img_counter:0>5}.png'), normalize=True, value_range=(-1, 1))
            img_counter += 1


def setup_evaluation(dataset_name, generated_dir, path, target_size=128, num_real_images=8000):
    ...
    if not os.path.exists(real_dir):
        ...
        dataloader, CHANNELS = datasets.get_dataset(dataset_name, img_size=target_size, path=path, real_pose=True)
...
if __name__ == '__main__':
    ...
    real_images_dir = setup_evaluation(config['dataset']['class'], opt.output_dir, target_size=config['global']['img_size'], num_real_images=opt.num_real_images, **config['dataset']['kwargs'])
    ...
    with torch.no_grad():
        ...
        for img_counter in tqdm(range(opt.num_images)):
            ...
            with torch.no_grad():
                ...
                save_image(img, os.path.join(opt.output_dir, f'{img_counter:0>5}.png'), normalize=True, value_range=(-1, 1))
