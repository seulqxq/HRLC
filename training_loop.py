                ...
                if rank == 0:
                    if i%10 == 0:
                        if imgs.shape[-1] == 64:
                            ...
                            tqdm.write(f"[Experiment: {opt.output_dir}] [GPU: {os.environ['CUDA_VISIBLE_DEVICES']}] [Step: {discriminator.step}] [D Total: {d_loss['Total']}] [D Main: {d_loss['Main']}] [D Patch: {d_loss['Patch']}] [G loss: {g_loss['Total']}] [G Main: {g_loss['Main']}] [G Patch: {g_loss['Patch']}] [Scale: {scaler.get_scale()}]")
                        # tqdm.write(f"[Experiment: {opt.output_dir}] [GPU: {os.environ['CUDA_VISIBLE_DEVICES']}] [Step: {discriminator.step}] [D loss: {d_loss}] [G loss: {g_loss}] [Scale: {scaler.get_scale()}]")
