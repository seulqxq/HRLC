...
class GramRF(nn.Module):
    def __init__(self, z_dim=100, feature_dim=0, hidden_dim=256, sigma_clamp_mode='softplus', rgb_clamp_mode='widen_sigmoid'):
        super().__init__()
        self.z_dim = z_dim
        self.feature_dim = feature_dim  # 特征平面
        ...
        # 辐射场生成网络 SIRNE-MLP 8层 与 GRAM 相同
        ...
        # 
        self.color_layer = nn.ModuleList([FiLMLayer(hidden_dim + 3, hidden_dim)])   # 加入了视角
    ...
    def forward_feature_with_frequencies_phase_shifts(self, input, frequencies, phase_shifts, ray_directions, eps=1e-3):
        ...
        return torch.cat([rgb, sigma], dim=-1), feature # 颜色、透明度、特征平面
...
class Gram(nn.Module):
    def __init__(self, z_dim=256, feature_dim=0, hidden_dim=256, sigma_clamp_mode='softplus', rgb_clamp_mode='widen_sigmoid', **sample_network_kwargs):
        super().__init__()
        self.sample_network = GramSample(**sample_network_kwargs)   # 采样网络
        self.rf_network = GramRF(z_dim, feature_dim, hidden_dim, sigma_clamp_mode, rgb_clamp_mode)  # 辐射生成网络（颜色、密度、特征平面）

    def get_avg_w(self):
        return self.rf_network.get_avg_w()

    def get_intersections(self, points, levels):    # 交点采样
        return self.sample_network.get_intersections(points, levels)

    def get_radiance(self, z, x, ray_directions, truncation_psi=1): # 辐射生成
