...
# 用于将 z 映射到 S_i 用到 RRDB 卷积层中
# map_output_dim: styles 输出维度
...
class ModConv2D(nn.Conv2d):
    def __init__(
        self,
        in_channels,
        out_channels,   # 上采样后的 输出通道数  -->  up_channels[0] * 4
        kernel_size,    # 卷积核大小 --> 3
        stride = 1,
        padding = 0,
        dilation = 1,
    ...
    def forward(self, x, styles):
        ...
        # 返回卷积结果 size: [batchsize, out_channels, x.shape[2], x.shape[3]]
...
args:
    ...
    scale_factor: 上采样因子
    up_channels: 上采样通道数
    up_sample_ks: 上采样卷积核大小
    to_rgb_ks: 输出卷积核大小
    use_mapping_network: 是否使用映射网络
    use_pixel_shuffle: 是否使用像素洗牌

'''

class StyleRRDBNet(nn.Module): 
    def __init__(self, in_nc, out_nc, w_dim, nf, nb, 
                 gc=32, scale_factor=4, up_channels=None, up_sample_ks=3, to_rgb_ks=3, 
                 use_mapping_network=False, use_pixel_shuffle=True):
        super(StyleRRDBNet, self).__init__()
        self.scale_factor = scale_factor
        self.use_pixel_shuffle = use_pixel_shuffle
        RRDB_block_f = functools.partial(RRDB, nf=nf, gc=gc)    # 残差-in-残差 密集块

        self.conv_first = nn.Conv2d(in_nc, nf, 3, 1, 1, bias=True)  # 第一层卷积

        # 定义 RRDB 层数 nb: 残差块的层数
        ...
        if self.use_pixel_shuffle:
            for i in range(self.num_upconvs):
                if i == 0:  
                    self.upconvs.append(ModConv2D(nf, up_channels[0] * 4, 3, 1, 1, bias=True))  # 上采样卷积层 定义
                    style_dim += nf
                else:
    ...
    def forward(self, x, w, truncation_psi=1):
        if self.use_mapping_network:
            ...
            style = self.affine(w)
        if truncation_psi < 1:
            style = self.avg_style.lerp(style, truncation_psi)  # lerp() 线性插值函数
