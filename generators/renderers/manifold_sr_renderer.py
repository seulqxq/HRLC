...
from ..super_resolutions.aaffblk import *
...
def fancy_integration(rgb_sigma, z_vals, is_valid, bg_idx, last_back=False, white_back=False, delta_alpha=0.04, delta_final=1e10):
    ...
    depth_maps = weights * z_vals
    if white_back:
        rgb_final = rgb_final + 1-weights_sum

    return rgb_final, depth_final, weights, T, depth_maps
...
class ManifoldSRRenderer:
    ...
    """
    def __init__(self, num_manifolds, levels_start, levels_end, num_samples, last_back=False, white_back=False, delta_alpha=0.04, delta_final=1e10, lock_view_dependence=True, scale_factor=4) -> None:
    ...
    def render(self, intersection, volume, feature, sr_model, sr_model_bg, img_size, camera_origin, camera_pos, fov, ray_start, ray_end, device):
        ...
        with torch.no_grad():
            ...
            # 低分辨率的像素颜色和深度
            lr_pixels, lr_depth, _, _, depth_maps = fancy_integration(raw, z_vals, is_valid=is_valid, bg_idx=bg_idx, white_back=self.white_back, last_back=self.last_back, delta_final=self.delta_final, delta_alpha=self.delta_alpha)
        ...
        # SR images
        aaffblk = create_model(scale_factor=self.scale_factor)
        aaffblk = aaffblk.to(device)
        sr_pixels = aaffblk(lr_pixels)
        ...
        # LR Feat. Maps N * 64 * 64 * (4 + 32) raw: 4 --> [rgb, alpha]
        # feature: (raw, feature) --> [rgb, alpha, feature] (3 + 1 + feature_dim)
        ...
        # SR manifolds
        # change
        # hr_raw: 高分辨率 [raw, feature] --> [B, rgb, alpha, feature] 
        depth_maps = depth_maps.permute(0, 2, 3, 1)
        depth_maps_bg = depth_maps[:, -1:, :, :]
        depth_maps = depth_maps[:, :-1, :, :]
        depth_maps = depth_maps.reshape(B*(self.num_manifolds-1), -1, HL, HL) # [B*(num_manifolds-1), 1, HL, HL]
        depth_maps_bg = depth_maps_bg.reshape(B, 1, HL, HL) # [B, 1, HL, HL]

        hr_raw = sr_model(lr_features, depth_maps) # [B*(num_manifolds-1), 4, H, H] sr_model: super_resolution --> StyleRRDBNet
        rgb = hr_raw[:, :3, :, :]
        alpha = torch.sigmoid(hr_raw[:, 3:, :, :])

        # hr_raw: [rgb, alpha] --> 用于渲染
        hr_raw = torch.cat([rgb, alpha], dim=1) # [B*(num_manifolds-1), 4, H, H]
        
        hr_raw_bg = sr_model_bg(lr_raw_bg, depth_maps_bg) # [B, 3, H, H]
        hr_raw_bg = torch.cat([hr_raw_bg, torch.zeros(B, 1, H, H, device=device)], dim=1) # [B, 4, H, H]

        # Rendering
        with torch.no_grad():   # 对采样点 进行 插值 
        ...
        # [rgb, alpha] bg
        raw_bg = F.grid_sample(hr_raw_bg.float(), bg_invmap(pts_bg[..., :2].float()).permute(0, 2, 1, 3).reshape(B, H, H, 2), 
                               mode='bilinear', padding_mode='reflection', align_corners=False).reshape(B, 1, 4, H*H).permute(0, 3, 1, 2) # [B, H*H, 1, 4]
        
        # [rgb, alpha] 
        raw = F.grid_sample(hr_raw.float(), pts[..., :2].float().permute(0, 2, 1, 3).reshape(B*(self.num_manifolds-1), H, H, 2) / fov * 100, 
                            mode='bilinear', padding_mode='zeros', align_corners=False).reshape(B, self.num_manifolds-1, 4, H*H).permute(0, 3, 1, 2) # [B, H*H, num_manifolds-1, 4]
        ...
        raw = torch.gather(raw, -2, indices.expand(-1, -1, -1, 4))
        is_valid = torch.gather(is_valid,-2,indices)
        bg_idx = torch.argmax(indices,dim=-2)   # argmax 返回最大值的索引
        ...
        return (hr_pixels, sr_pixels, lr_pixels, hr_raw, lr_raw), detail
