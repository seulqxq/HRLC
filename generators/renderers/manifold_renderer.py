...
def get_intersection_with_MPI(transformed_ray_directions,transformed_ray_origins,device, mpi_start=0.12,mpi_end=-0.12,mpi_num=24):
    ...
    z_vals = z_vals/transformed_ray_directions[...,-1:] #[batch,N,mpi_num]  归一化深度差值
...
class ManifoldRenderer:
    ...
    """
    def __init__(self, num_manifolds, levels_start, levels_end, num_samples, last_back=False, white_back=False, delta_alpha=0.04, delta_final=1e10, lock_view_dependence=True) -> None:
    ...
    def render(self, intersection, volume, img_size, camera_origin, camera_pos, fov, ray_start, ray_end, device):
        ...
        with torch.no_grad():
            # points, z_vals, rays_d_cam
            pts_sample, z_vals, rays_d = get_initial_rays(B, self.num_samples, resolution=(H, H), 
                                                          device=device, fov=fov, ray_start=ray_start, ray_end=ray_end, randomize=False)
            pts_sample, rays_d, rays_o, _ = transform_sampled_points(pts_sample, rays_d, camera_origin, camera_pos, device=device)
            
            pts_sample = pts_sample.reshape(B, H*H, -1, 3)  # 采样点大小 [batch_size, H*H, -1, 3]
            levels = torch.linspace(self.levels_start, self.levels_end, self.num_manifolds-1).to(device)

            # 最后背景的点 points, z_vals
        ...
        # rgb_final, depth_final, weights, T
        pixels, depth, _, _ = fancy_integration(raw, z_vals, is_valid=is_valid, bg_idx=bg_idx, 
                                                white_back=self.white_back, last_back=self.last_back, 
                                                delta_final=self.delta_final, delta_alpha=self.delta_alpha)
        ...
        return pixels, detail   # pixels: [batch_size, 3, H, H]
