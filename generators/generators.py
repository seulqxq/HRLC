...
class GramGenerator(Generator):
    ...
    Args:
        ...
        sample_dist (str): Camera sampling distribution 相机采样分布
    ...
    def __init__(self, z_dim, img_size, h_stddev, v_stddev, h_mean, v_mean, sample_dist, representation_kwargs, renderer_kwargs):
        ...
        self.h_mean = h_mean
        self.v_mean = v_mean
        self.sample_dist = sample_dist  # 相机采样分布
        self.representation = Gram(z_dim, **representation_kwargs)  # GRAM 辐射流形生成器网络
        self.renderer = ManifoldRenderer(**renderer_kwargs) # 辐射流形渲染器

    def _volume(self, z, truncation_psi=1): # GramRF [rgb, alpha]
        return lambda points, ray_directions: self.representation.get_radiance(z, points, ray_directions, truncation_psi)   

    def _intersections(self, points, levels):   # 采样点 [ntersections, s, is_valid]
    ...
    def forward(self, z, fov, ray_start, ray_end, img_size=None, camera_origin=None, camera_pos=None, truncation_psi=1):
        if camera_origin is None:
            camera_origin, camera_pos = sample_camera_positions(z.device, z.shape[0], 1, 
                                                                self.h_stddev, self.v_stddev, self.h_mean, self.v_mean, self.sample_dist)
        else:
            camera_origin = torch.tensor(camera_origin, dtype=torch.float32, device=z.device).reshape(1, 3).expand(z.shape[0], 3)
        if img_size is None:
            img_size = self.img_size
        img, _ = self.renderer.render(self._intersections,  # 采样点 
                                      self._volume(z, truncation_psi),  # [rgb, alpha] 
                                      img_size, camera_origin, camera_pos, fov, ray_start, ray_end, z.device)
...
class GramHDGenerator(Generator):
    ...
    def __init__(self, z_dim, feature_dim, img_size, h_stddev, v_stddev, h_mean, v_mean, sample_dist, representation_kwargs, super_resolution_kwargs, renderer_kwargs, gram_model_file=None, lr_img_size=None):
        ...
        # 超分辨率网络（前 N - 1）  use_mapping_network --> 是否使用映射网络（StyleGAN）
        ...
        self.renderer = ManifoldSRRenderer(scale_factor = self.scale_factor, **renderer_kwargs) # 高分辨率渲染器
    ...
    def _super_resolution(self, z, truncation_psi=1):
        return lambda x, y: self.super_resolution(x, y, z, truncation_psi)
    ...
    def forward(self, z, fov, ray_start, ray_end, img_size=None, camera_origin=None, camera_pos=None, truncation_psi=1):
        ...
        img, _ = self.renderer.render(self._intersections,  # 交点（采样点）
                                      self._volume(frequencies, phase_shifts, truncation_psi),      # [rgb, alpha]
                                      self._feature(frequencies, phase_shifts, truncation_psi),     # [rgb, alpha], feature
                                      self._super_resolution(z, truncation_psi), 
                                      self.super_resolution_bg, 
                                    #   self.aaffblk,
                                      img_size, camera_origin, camera_pos, fov, ray_start, ray_end, z.device)
        return img, camera_pos
