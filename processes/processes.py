...
class SRGan3DProcess:
    ...
    def train_D(self, real_imgs, real_positions, generator_ddp, discriminator_ddp, optimizer_D, scaler, config, device):
        ...
        with torch.cuda.amp.autocast():
            ...
            with torch.no_grad():
                ...
                for split in range(self.batch_split):
                    subset_z = z[split * split_batch_size:(split+1) * split_batch_size]
                    (g_imgs, sr_imgs, _, _, _), g_pos = generator_ddp(subset_z, **config['camera'])
                    gen_imgs.append(g_imgs)
                    gen_positions.append(g_pos)
                gen_imgs = torch.cat(gen_imgs, axis=0)
                gen_positions = torch.cat(gen_positions, axis=0)
        ...
        return {'Total': round(d_loss.item(), 8), 'Main': round(main_loss.item(), 8), 'Patch': round(patch_loss.item(), 8), 'R1': round(grad_penalty.item(), 8), 'Pos': round(position_penalty.item(), 8)}
    ...
    def train_G(self, real_imgs, generator_ddp, ema, ema2, discriminator_ddp, optimizer_G, scaler, config, device):
        ...
        for split in range(self.batch_split):
            with torch.cuda.amp.autocast():
                subset_z = z[split * split_batch_size:(split+1) * split_batch_size]
                (gen_imgs, sr_imgs, lr_imgs, sr_rgba, lr_rgba), gen_positions = generator_ddp(subset_z, **config['camera'])
                ...
                # sr_loss = nn.MSELoss()(sr_imgs, gen_imgs)
                # print('sr_loss: ', sr_loss)
                cons_penalty += self.cons_lambda * (nn.MSELoss()(sr_imgs, gen_imgs) + 1e-8)
                # lr_loss
                real_img = real_imgs[split * split_batch_size:(split+1) * split_batch_size]
                real_imgs.requires_grad = True
                size = real_img.shape[2] // generator_ddp.module.scale_factor
                lr_real_imgs = F.interpolate(real_img, size=[size, size], mode='bicubic', align_corners=True)
                lr_loss = nn.MSELoss()(lr_imgs, lr_real_imgs) + 1e-8

                sr_loss = nn.MSELoss()(sr_imgs, real_img) + 1e-8

                patch_loss = 0
                if self.use_patch_d:
                    patch_loss = self.patch_lambda * F.softplus(-g_patch_preds).mean()
                main_loss = F.softplus(-g_preds).mean()
                g_loss = main_loss + identity_penalty + cons_penalty + patch_loss + sr_loss
            ...
            log['Total'], log['Main'], log['Patch'], log['Cons'], log['Pos'] = round(log['Total'], 8), round(log['Main'], 8), round(log['Patch'], 8), round(log['Cons'], 8), round(log['Pos'], 8) 
    ...
    def snapshot(self, generator_ddp, discriminator_ddp, config, output_dir, device, batchsize=1):
        with torch.no_grad():
            ...
            for i in range(0, 25, batchsize):
                ...
                lr_gen_imgs.append(imgs[2])
                sr_gen_imgs.append(imgs[1])
                hr_gen_imgs.append(imgs[0])
            lr_gen_imgs = torch.cat(lr_gen_imgs, dim=0)
            sr_gen_imgs = torch.cat(sr_gen_imgs, dim=0)
            hr_gen_imgs = torch.cat(hr_gen_imgs, dim=0)
            save_image(lr_gen_imgs[:25], os.path.join(output_dir, "%06d_fixed_lr.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
            save_image(sr_gen_imgs[:25], os.path.join(output_dir, "%06d_fixed.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
            save_image(hr_gen_imgs[:25], os.path.join(output_dir, "%06d_fixed_sr.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
            ...
            sr_gen_imgs = torch.cat(sr_gen_imgs, dim=0)
            hr_gen_imgs = torch.cat(hr_gen_imgs, dim=0)
            save_image(lr_gen_imgs[:25], os.path.join(output_dir, "%06d_tilted_lr.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
            save_image(sr_gen_imgs[:25], os.path.join(output_dir, "%06d_tilted.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
            save_image(hr_gen_imgs[:25], os.path.join(output_dir, "%06d_tilted_sr.png"%discriminator_ddp.module.step), nrow=5, normalize=True, value_range=(-1, 1))
