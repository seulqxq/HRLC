...
if __name__ == '__main__':
    ...
    parser.add_argument('--real_images_cache_name', type=str, default='real_images', help='Real images cache name')
    parser.add_argument('--fake_images_cache_name', type=str, default='fake_images', help='Fake images cache name')
    ...
    if opt.use_real_images_cache:
        ...
        if not has_cache(opt.tmp_dir, opt.real_images_cache_name):
            sample_from_real_image(opt.real_images_dir, tmp_real_dir, opt.image_size, opt.num_samples)
    ...
    metrics_dict = calculate_metrics(tmp_fake_dir, tmp_real_dir, cuda=True, isc=True, fid=True, kid=True, verbose=True,
    ...
    print(metrics_dict)
    with open(os.path.join(os.path.dirname(__file__), f'{tmp_fake_dir}.txt'), 'w') as f:
